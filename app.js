...
function segLabel(o){ return [o.track,o.region,o.cohort].filter(x=>x&&x!=="").join(" / "); }
// 조사에 해당하는 커리큘럼 수업 (트랙+지역+기수 정확 일치 + 날짜)
function surveyLessons(survey, curriculum){
  return curriculum.filter(c=> c.track===survey.track && (c.region||"")===(survey.region||"")
      && (c.cohort||"")===(survey.cohort||"") && dateInScope(survey,c))
    .sort((a,b)=>(a.date||"").localeCompare(b.date||"") || (a.title||"").localeCompare(b.title||""));
}
// 학생이 볼 수 있는 조사인가 (트랙+지역+기수 일치)
function studentCanSee(s){
  return s.track===session.track && (s.region||"")===(session.region||"") && (s.cohort||"")===(session.cohort||"");
}
...
// draft(전체 허용)와 커리큘럼 한 건의 조합 매칭
function comboMatch(draft,c){
  return (draft.track==="전체"||c.track===draft.track)
    && (draft.region==="전체"||(c.region||"")===draft.region)
...
async function doLogin(){
  ...
  session={role:"student",studentId:st.studentId,name:st.name,track:st.track||"공통",region:st.region||"",cohort:st.cohort||""};
  enterApp();
}
function logout(){
  session=null;
...
function enterApp(){
  ...
  document.getElementById("whoText").textContent =
    ...
    : (session.name+" ("+session.studentId+" · "+segLabel(session)+")");
  hide("loginView");
...
async function loadCurriculum(){
  const list=(await Data.listCurriculum()).sort((a,b)=>(a.date||"").localeCompare(b.date||"")||segLabel(a).localeCompare(segLabel(b)));
  ...
  let h='<table><tr><th>회차</th><th>날짜</th><th>트랙</th><th>지역</th><th>기수</th><th>제목</th><th></th></tr>';
  list.forEach(c=>{
    h+='<tr><td class="center">'+(rmap[c.date]?rmap[c.date]+'회':'-')+'</td><td>'+esc(c.date)+'</td>'+
      '<td><span class="pill pill-track">'+esc(c.track)+'</span></td><td>'+esc(c.region||'-')+'</td><td>'+esc(c.cohort||'-')+'</td>'+
      '<td>'+esc(c.title)+'</td>'+
      '<td><button class="btn btn-danger btn-sm" onclick="delCurriculum(\''+c.id+'\')">삭제</button></td></tr>';
  });
  el.innerHTML=h+'</table>';
}
async function delCurriculum(id){ if(!confirm("이 수업을 삭제할까요?"))return; await Data.deleteCurriculum(id); loadCurriculum(); }
async function clearCurriculum(){ if(!confirm("불러온 수업 데이터를 전체 삭제할까요?"))return; await Data.clearCurriculum(); loadCurriculum(); }
  ...
  readSheet(f, async rows=>{
    ...
    for(const r of rows){
      ...
      await Data.addCurriculum({
        ...
        region:(r["지역"]||r["region"]||"").toString().trim(),
        cohort:(r["기수"]||r["cohort"]||"").toString().trim(),
        title:(r["제목"]||r["title"]||"").toString(),
        content:(r["내용"]||r["content"]||"").toString(),
        createdAt:new Date().toISOString()
      });n++;
    }
    msg("curMsg",n+"건의 수업 데이터를 파이어베이스에 저장했습니다.","ok"); loadCurriculum();
  });
  ev.target.value="";
}
function downloadCurriculumTemplate(){
  const ws=XLSX.utils.aoa_to_sheet([
    ["트랙","지역","기수","날짜","제목","내용"],
    ["AX","부산","1기","2026-07-14","머신러닝 모델링 실습","1기(A강사) 수업"],
    ["AX","부산","2기","2026-07-14","머신러닝 모델링 실습","2기(B강사) 수업"],
    ["스마트팩토리","대구","1기","2026-07-14","제조 데이터 분석 실습","설비 데이터 수집과 이상탐지"]
  ]);
...
// 지역/기수 select 를 커리큘럼의 실제 값으로 채움
async function populateSurveyDims(){
  const cur=await Data.listCurriculum();
  fillSelect("sgRegion",[...new Set(cur.map(c=>c.region||"").filter(x=>x!==""))]);
  fillSelect("sgCohort",[...new Set(cur.map(c=>c.cohort||"").filter(x=>x!==""))]);
}
function fillSelect(id,values){
  const sel=document.getElementById(id); if(!sel)return;
  const cur=sel.value||"전체";
  sel.innerHTML='<option value="전체">전체</option>'+values.sort().map(v=>'<option>'+esc(v)+'</option>').join("");
  // 이전 선택값 유지(있으면)
  if([...sel.options].some(o=>o.value===cur)) sel.value=cur;
}
function ensureOption(id,v){
  const sel=document.getElementById(id); if(!sel||!v)return;
...
function currentSurveyDraft(){
  ...
  const base={type:type, track:val("sgTrack"), region:val("sgRegion"), cohort:val("sgCohort")};
...
async function updateSurveyPreview(){
  ...
  const scope=cur.filter(c=>dateInScope(draft,c) && comboMatch(draft,c));
  if(!scope.length){el.innerHTML="이 조건에 해당하는 수업이 <b>없습니다.</b>";return;}
  const groups={};
  scope.forEach(c=>{ const k=comboKey(c); (groups[k]=groups[k]||{seg:segLabel(c),n:0}).n++; });
  const keys=Object.keys(groups);
  el.innerHTML=(keys.length>1?keys.length+"개 조사가 조합별로 생성됩니다 → ":"대상 → ")+
    keys.map(k=>esc(groups[k].seg)+"("+groups[k].n+"건)").join(", ");
}
function autoTitle(round,seg,draft){
  const scope=draft.type==="주간"?(draft.startDate+"~"+draft.endDate+" 주간"):(draft.date+" 일간");
  return (round?round+"회 ":"")+"["+seg+"] "+scope+" 만족도";
}
async function saveSurvey(){
  ...
  /* 수정 모드 */
  if(editingSurveyId){
    if(draft.track==="전체"||draft.region==="전체"||draft.cohort==="전체"){
      msg("surveyMsg","수정 시에는 트랙·지역·기수를 구체적으로 선택하세요.","err");return;
    }
  ...
  let combos=[];
  const seen={};
  scope.forEach(c=>{ const k=comboKey(c); if(!seen[k]){seen[k]=1;combos.push({track:c.track,region:c.region||"",cohort:c.cohort||""});} });
  if(!combos.length){
    if(draft.track!=="전체"&&draft.region!=="전체"&&draft.cohort!=="전체"){
      if(!confirm("해당 조건에 수업 데이터가 없습니다. 그래도 빈 조사를 생성할까요?"))return;
      combos=[{track:draft.track,region:draft.region,cohort:draft.cohort}];
    }else{ msg("surveyMsg","해당 날짜에 수업이 없습니다. 트랙/지역/기수를 구체적으로 지정하거나 수업 데이터를 확인하세요.","err");return; }
  }
  let created=0;
...
async function editSurvey(id){
  ...
  ensureOption("sgRegion",s.region||""); ensureOption("sgCohort",s.cohort||"");
  document.getElementById("sgRegion").value=s.region||"전체";
  document.getElementById("sgCohort").value=s.cohort||"전체";
  if(!s.region) document.getElementById("sgRegion").value="전체";
  if(!s.cohort) document.getElementById("sgCohort").value="전체";
  toggleSurveyDateMode();
...
async function loadSurveyList(){
  ...
  let h='<table><tr><th>회차</th><th>구분</th><th>트랙</th><th>지역</th><th>기수</th><th>날짜/기간</th><th>수업</th><th>상태</th><th></th></tr>';
  surveys.forEach(s=>{
    ...
    h+='<tr><td class="center">'+(s.round?esc(s.round)+'회':'-')+'</td>'+
      ...
      '<td><span class="pill pill-track">'+esc(s.track)+'</span></td><td>'+esc(s.region||'-')+'</td><td>'+esc(s.cohort||'-')+'</td>'+
...
async function addStudent(){
  const st={studentId:val("stId").trim(),name:val("stName").trim(),track:val("stTrack"),
    region:val("stRegion").trim(),cohort:val("stCohort").trim(),createdAt:new Date().toISOString()};
  ...
  document.getElementById("stRegion").value="";document.getElementById("stCohort").value="";
  loadStudents();
}
async function loadStudents(){
  ...
  let h='<table><tr><th>학번</th><th>이름</th><th>트랙</th><th>지역</th><th>기수</th><th></th></tr>';
  list.sort((a,b)=>a.studentId.localeCompare(b.studentId)).forEach(s=>{
    h+='<tr><td>'+esc(s.studentId)+'</td><td>'+esc(s.name)+'</td><td>'+esc(s.track)+'</td>'+
      '<td>'+esc(s.region||'-')+'</td><td>'+esc(s.cohort||'-')+'</td>'+
...
function downloadStudentTemplate(){
  const ws=XLSX.utils.aoa_to_sheet([["학번","이름","트랙","지역","기수"],
    ["20250001","홍길동","AX","부산","1기"],["20250002","김철수","AX","부산","2기"]]);
...
async function loadResultSurveys(){
  ...
  sel.innerHTML='<option value="">— 선택하세요 —</option>'+
    surveys.map(s=>'<option value="'+s.id+'">'+(s.round?s.round+'회 · ':'')+esc(segLabel(s))+' · '+esc(surveyScope(s))+' · '+esc(s.type)+(s.active===false?' (오프)':'')+'</option>').join("");
...
async function loadStudent(){
  ...
  document.getElementById("surveyForm").innerHTML= list.length?"":
    '<p class="muted" style="margin-top:12px">현재 내 조건('+esc(segLabel(session))+')에 열려 있는 만족도 조사가 없습니다.</p>';
}
...
async function renderSurveyForm(){
  ...
  if(!studentCanSee(s)){box.innerHTML='<div class="notice">본인(트랙·지역·기수) 대상 조사가 아닙니다.</div>';return;}
  ...
  let h='<div class="notice"><b>'+(s.round?s.round+'회 · ':'')+esc(s.title)+'</b> · '+esc(surveyScope(s))+' · '+esc(segLabel(s))+'<br>'+
...
(function(){
  ...
  if(useCloud){badge.innerHTML="연동되었습니다";
    foot.textContent="연동 · Let's Grow with LG전자 만족도 조사";}
  else{badge.innerHTML="⚠ 현재 <b>로컬 저장 모드</b>입니다. app.js의 firebaseConfig를 입력하면 클라우드 저장이 활성화됩니다.";
    foot.textContent="로컬(브라우저) 저장 모드 · 설정 후 연동";}
